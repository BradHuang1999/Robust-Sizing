...
SimulationResult snc_lolp(vector<double> &load_trace, vector<double> &solar_trace,
    ...
    while (cells_U - cells_L > cells_step) {
        ...
        snc_lolp_core(load_trace, solar_trace, start_indices, end_indices, epsilon, confidence, traceLength, pv_max,
                      losses);
    ...
    for (double cells = starting_cells; cells <= cells_max; cells += cells_step) {
        ...
        while (true) {
            ...
            snc_lolp_core(load_trace, solar_trace, start_indices, end_indices, epsilon, confidence, traceLength,
