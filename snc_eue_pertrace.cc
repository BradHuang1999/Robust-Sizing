...
void compute_I(vector<double> &load_trace, vector<double> &solar_trace,
    ...
    for (int trace = 0; trace < numTraces; trace++) {
        ...
        for (int k = 0; k < traceLength; k++) {
            ...
            P_net[trace][k] = (1 / eta_d) * (eta_d * eta_c * P_in[trace][k] -
                                             P_out[trace][k]); //Yashar: defined as the round-trip efficiency
    ...
    for (int trace = 0; trace < numTraces; trace++) {
        ...
        for (int j = 1; j < traceLength; j++) {
            ...
            Y[trace][j] = beta_l * P_out[trace][j] - P_net[trace][j] +
                          fmax(Y[trace][j - 1] - beta_l * (P_out[trace][j - 1]),
                               0); // TODO: this used to be max (...., -sigma). is it right to replace it with 0?
    ...
    for (int trace = 0; trace < numTraces; trace++) {
        ...
        if ((pI[trace] > pII[trace]) && (lambdaI[trace] > lambdaII[trace])) {
            EUL_temp = pII[trace] / lambdaII[trace] *
                       (1 - pow(pII[trace] / pI[trace], lambdaII[trace] / (lambdaI[trace] - lambdaII[trace]))) +
                       pI[trace] / lambdaI[trace] *
                       pow(pII[trace] / pI[trace], lambdaI[trace] / (lambdaI[trace] - lambdaII[trace]));
            //cout << "1" << endl;
        } else if ((pII[trace] > pI[trace]) && (lambdaII[trace] > lambdaI[trace])) {
            EUL_temp = pI[trace] / lambdaI[trace] *
                       (1 - pow(pI[trace] / pII[trace], lambdaI[trace] / (lambdaII[trace] - lambdaI[trace]))) +
                       pII[trace] / lambdaII[trace] *
                       pow(pI[trace] / pII[trace], lambdaII[trace] / (lambdaII[trace] - lambdaI[trace]));
            //cout << "2" << endl;
...
SimulationResult snc_eue(vector<double> &load_trace, vector<double> &solar_trace,
    ...
    while (cells_U - cells_L > cells_step) {
        ...
        snc_eue_core(load_trace, solar_trace, start_indices, end_indices, epsilon, confidence, traceLength, pv_max,
                     losses);
    ...
    for (double cells = starting_cells; cells <= cells_max; cells += cells_step) {
        ...
        while (true) {
            ...
            snc_eue_core(load_trace, solar_trace, start_indices, end_indices, epsilon, confidence, traceLength,
                         lowest_feasible_pv - pv_step, losses);
            bool valid = check_losses(losses, numTraces, epsilon, confidence);
            if (valid) {
